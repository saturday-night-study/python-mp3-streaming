...
class TestFileIO(unittest.TestCase):
    ...
    def test_open_exists_file(self):
        fio = FileIO(self.__exists_input_path, "rb")
        self.assertFalse(fio.closed)

    # 존재하지 않는 파일이 입력된 경우 Error를 발생시키는지 확인
    def test_open_not_exists_file(self):
        self.assertRaises(FileNotFoundError, FileIO, self.__not_exists_input_path, "rb")

    # 파일 경로가 문자열이 아닌 타입으로 입력된 경우 Error를 발생시키는지 확인
    def test_open_invalid_parameter_type(self):
        self.assertRaises(ValueError, FileIO, 123, "rb")

    # 파일 경로가 디렉터리일 경우 Error를 발생시키는지 확인
    def test_open_directory(self):
        self.assertRaises(IsADirectoryError, FileIO, self.__directory_input_path, "rb")
    ...
    def test_read_eof(self):
        fio = FileIO(self.__empty_input_path, "rb")
